...
from .accountants import create_accountant, create_accountant_fl, IAccountant, FedDPAccountant
...
class PrivacyEngine:
    ...
    def _prepare_data_loader(
        ...
        if poisson_sampling:
            ...
            if accountant.mechanism() == "fed_dp":
    ...
    def prepare_fed_dp(
        ...
        if self.accountant.mechanism() == "fed_dp":
            self.accountant.init(budget=budgets[0], sample_rate=sample_rate, eta=eta, delta_g=delta_g)
        elif self.accountant.mechanism() == "dpw":
            for i, acct in enumerate(self.accountant.accountants):
                acct.init(budget=budgets[i], sample_rate=sample_rate, eta=eta, delta_g=delta_g)

    def make_private_with_fed_dp(
            self,
            module: nn.Module,
            optimizer: optim.Optimizer,
            data_loader: DataLoader,
            *,
            accountant: FedDPAccountant,
