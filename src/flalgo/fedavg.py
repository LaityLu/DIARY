...
from opacus.accountants.utils import GENERATE_EPSILONS_FUNC, get_noise_multiplier_with_fed_dp
...
class FedAvg:
    ...
    def __init__(
        ...
        if config['FL']['with_DP']:
            self.privacy_engine = PrivacyEngine(accountant="fed_dp", n_clients=config['FL']['num_clients'])
    ...
    def initial_clients(self):
        ...
        if self.privacy_engine is not None:
            ...
            budgets_per_client = self.generate_budgets()
            ...
            for target_epsilon in budgets_per_client:
                noise = get_noise_multiplier_with_fed_dp(
                    ...
                    sample_rate=self.config['Fed_dp']['sample_rate'],
                    delta=self.config['Fed_dp']['delta'],
                    delta_g=self.config['Fed_dp']['delta_g'],
                    eta=self.config['Fed_dp']['eta'],
                    noise_config=self.config['Fed_dp']['noise_config']
                )
            ...
            self.privacy_engine.prepare_fed_dp(
    ...
    def generate_budgets(self) -> List[float]:
        BoundedFunc = lambda values: [min(max(x, self.config['Fed_dp']['budgets_setting']['min_epsilon']),
                                          self.config['Fed_dp']['budgets_setting']['max_epsilon'])
                                      for x in values]
        budgets_per_client = BoundedFunc(
            GENERATE_EPSILONS_FUNC[self.config['Fed_dp']['budgets_setting']['name']](
                self.config['FL']['num_clients'], self.config['Fed_dp']['budgets_setting']['args']
            )
        )
        return budgets_per_client
