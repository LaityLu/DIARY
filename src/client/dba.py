...
class DBA(Client):
    ...
    def make_private(self,
        ...
        if self.with_dp:
            ...
            self.model, self.optimizer, self.train_dl = privacy_engine.make_private_with_fed_dp(
                module=self.model,
    ...
    def local_train(self) -> Tuple[torch.nn.Module, float]:
        ...
        if len(self.adversary_list) < self.trigger_args["trigger_num"]:
            adversarial_index = random.choice(list(range(self.trigger_args["trigger_num"])))
        else:
