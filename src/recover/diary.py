...
from opacus.accountants.utils import get_noise_multiplier_with_fed_dp_recover
...
class DIARY(FedEraser):
    ...
    def recover(self):
        ...
        logger.info(f'DIARY select rounds: {self.list_select_rounds}')
        logger.info(f'DIARY select clients: {self.list_select_clients}')
        ...
        if self.recalculate_noise:
            ...
            for client_id, rounds in sel_counter.items():
                ...
                noise = get_noise_multiplier_with_fed_dp_recover(
    ...
    def adaptive_recover(self, old_global_models_state_dict, old_client_models_state_dict):
        ...
        for rd in range(len(self.list_select_rounds)):
            ...
            # begin training
            logger.info("----- DIARY Recover Round {:3d}  -----".format(rd))
